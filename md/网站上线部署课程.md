...
# 启动 Nginx
nginx   或者     sudo systemctl start nginx
# 配置防火墙规则
...
例如：scp ./dist.zip root@175.24.164.69:~/
...
server {
    listen       80;
    server_name  buouyu.top;

     location / {
        proxy_pass  http://127.0.0.1:9000;
                }
 return 301 https://buouyu.top$request_uri;
}
...
ssl_certificate /web/ssl/buouyu.top_bundle.crt;
ssl_certificate_key /web/ssl/buouyu.top.key;
...
## 数据库文件的导入和导出
```
mysqldump -h localhost -uroot -p123456 words > C:/Users/lby/Desktop/test.sql     导出
mysql -h localhost -uroot -p123456 test< C:/Users/lby/Desktop/test.sql   导入
mysql -uroot -p123456

linux
mysql -p Duyi_666duyi
mysql -u root -p Duyi_666duyi words < test.sql 
mysql -u root -p Duyi_666duyi –force words < test.sql   (强行导入)
SELECT english from cet4s;
mysql -u root -pDuyi_666duyi < test.sql
mysql -uroot -p words < test.sql
buouyu.top/api/CET4/get-all
http://localhost:9000/api/CET4/get-all
